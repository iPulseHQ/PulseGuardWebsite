...
const DATABASE_ID = process.env.NOTION_CHANGELOG_DATABASE_ID || '2ca4306c6bc6815882a2f4985ed7e029';
...
async function fetchNotionDatabase(forceRefresh?: boolean): Promise<NotionPage[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = `Notion API error (${response.status}): ${response.statusText}${body ? ` - ${body}` : ''}`;
      console.error(errorMessage);

      // If database not found, provide helpful error message
      if (response.status === 404) {
        throw new Error(`Database not found. Make sure the database is shared with your Notion integration. Database ID: ${DATABASE_ID}`);
      }
...
// Fetch available app options from Notion database
async function fetchAvailableApps(): Promise<string[]> {
  if (!NOTION_API_KEY) {
    return [];
  }
  ...
  try {
    ...
    const data = await response.json();
    const appProperty = data.properties?.App;

    if (appProperty?.type === 'select' && appProperty.select?.options) {
      return appProperty.select.options.map((option: { name: string }) => option.name);
    }
...
export async function GET(request: Request) {
  ...
  const appsOnly = searchParams.get('appsOnly') === 'true';
  const limit = limitParam ? Math.max(1, Math.min(200, Number(limitParam))) : undefined;

  if (!NOTION_API_KEY) {
    console.error('NOTION_API_KEY is not set');
    return jsonWithCors(
      { error: 'NOTION_API_KEY is not set on the server', entries: [], apps: [] },
      { status: 500 }
    );
  }

  if (!DATABASE_ID) {
    console.error('NOTION_CHANGELOG_DATABASE_ID is not set');
    return jsonWithCors(
      { error: 'NOTION_CHANGELOG_DATABASE_ID is not set on the server', entries: [], apps: [] },
      { status: 500 }
    );
  }

  // If requesting only available apps
  if (appsOnly) {
    const apps = await fetchAvailableApps();
    return jsonWithCors({ apps });
  }
